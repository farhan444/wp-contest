jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					shoptimizerAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			shoptimizerGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				shoptimizerGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			shoptimizerGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	$('.customize-control-sortable-repeater-add').click(function(event) {
		event.preventDefault();
		shoptimizerAppendRow($(this).parent());
		shoptimizerGetAllInputs($(this).parent());
	});
	...
	function shoptimizerAppendRow($element, defaultValue = '') {
	...
	function shoptimizerGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  shoptimizerGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function shoptimizerGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			shoptimizerGetAllPillCheckboxes($(this).parent());
		}
	});

	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		shoptimizerGetAllPillCheckboxes($(this).parent().parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function shoptimizerGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.shoptimizerfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.shoptimizerfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.shoptimizerfontslist[index].category);

		shoptimizerGetAllSelects($(this).parent().parent());
	});
	...
	function shoptimizerGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].shoptimizertinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].shoptimizertinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].shoptimizermediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['shoptimizer-upsell'] = api.Section.extend( {
